...
1. `DataSource`, `LibrarySourceRecord`와 별도 `sources` 앱은 제거한다. 수집 출처는 필요한 엔터티에 `provider_code`, `source_url`, `reference_date`, `fetched_at` 등의 값으로 직접 보존하되, 수집·적재 실행 결과는 `integrations.SourceSyncRun`으로 관리한다. `SourceSyncRun`은 도메인 데이터의 검수 상태가 아니라 실행 성공·실패와 처리 건수를 기록한다.
...
33. 시설·이미지·공휴일 등 서비스 테이블에는 `draft`, `verified`, `verification_status`, `is_verified`, `verified_at`, `verified_by` 같은 데이터 검수 상태를 두지 않는다. 시설의 미확인 값은 `NULL`, 미수집은 프로필·관계 행 부재로 표현하고, 수집 성공·실패와 처리 건수는 `SourceSyncRun` 및 reject·warning report에서 관리한다.
34. 이미지 라이선스 정보가 비어 있거나 공개 사용 조건이 불명확한 외부 이미지는 공개 대표 이미지로 활성화하지 않고 대체 이미지를 사용한다. 동일 URL이 여러 도서관에 연결되면 데이터 품질 경고를 남긴다.
35. GMS는 사실 데이터 수집원이나 추천 순위 결정기로 사용하지 않는다. 추후 생성형 AI 기능을 붙이더라도 Django 서버를 경유하는 표현 보조 기능으로 한정한다.
36. 프로그램 데이터의 `application_required`, 신청 시작·종료일, `application_status`, `operation_status`를 `Program`에 반영한다. `application_status`와 `operation_status`는 원천 고정값이나 별도 `*_raw`/`effective_*` 쌍이 아니라, 조회 시 날짜를 기준으로 재계산해 저장하는 캐시성 현재 상태다.
37. `application_status=신청마감`은 정원·인원 마감이 아니라 **신청 가능 기간 종료**를 뜻한다. 신청 시작 전 또는 판단 근거가 부족한 경우는 `NULL`로 둔다.
...
14. 프로그램은 조회·검색·저장 대상이며, 서비스 내부 신청·예약·결제·참여 이력을 관리하지 않는다. `application_status`와 `operation_status`는 신청·운영 날짜를 근거로 목록·상세 조회 전에 재계산하고 필요한 경우 DB 값을 갱신한다.
...
19. 정규화가 끝난 시설 파일의 한 행은 `LibraryFacilityProfile` 한 행과 대응한다. 필드가 명시적으로 `true/false`이면 `True/False`, 필드 누락은 `NULL`, 도서관 자체의 시설 행 부재는 프로필 부재로 표현한다. 수집 파일의 완성도와 매칭 실패는 프로필 상태값이 아니라 `SourceSyncRun`·reject report·별도 체크리스트에서 관리한다.
...
28. `SourceSyncRun`은 파일 import와 API 동기화의 실행 단위만 기록한다. 도메인 행의 신뢰도·검수 완료 여부를 나타내지 않으며, 상세 미매칭 목록은 별도 report 파일이나 구조화 로그에 둔다.
29. `application_status`의 값은 `신청가능`, `신청마감`, `신청없음`, `NULL`이며 `신청마감`은 신청 종료일 경과를 뜻한다. `operation_status`의 값은 `예정`, `진행중`, `종료`, `NULL`이다.
...
| `integrations` | `SourceSyncRun` | 외부 API·파일 수집 및 적재 실행의 성공·실패·처리 건수 기록 |

`integrations` 앱은 외부 API client, 파일 loader, normalizer, import service와 `SourceSyncRun`만 소유한다. 별도 `sources` 앱, `DataSource`, `LibrarySourceRecord`는 만들지 않는다.
...
| 시설 데이터·운영자 보정 | 정규화된 JSON 배열/JSONL에서 선택적 1:1 nullable boolean 프로필 | 행이 있으면 `True/False/NULL`을 그대로 upsert하고, 행이 없으면 프로필을 만들지 않음 | `LibraryFacilityProfile` |
...
| 수집·적재 실행 이력 | API 호출 또는 파일 import 1회당 실행 로그 | 성공·실패·부분 성공, 대상 연·월, 처리 건수와 대표 오류 저장 | `SourceSyncRun` |

별도 raw staging 테이블은 두지 않는다. 실행 요약은 `SourceSyncRun`, 상세 실패 행·미매칭 목록·merge 후보는 관리 명령 출력·구조화 로그·관리자용 report 파일로 남긴다.
...
`original_url`과 `file` 중 적어도 하나가 존재해야 한다. 외부 URL의 라이선스 정보가 비어 있거나 공개 사용 조건이 불명확하면 asset은 보존할 수 있으나 `is_active=False`로 두고 공개 대표 이미지 해석에서 제외한다. 이 판단 과정은 별도 검수 상태 필드가 아니라 import report와 asset의 공개 사용 여부로 관리한다.
...
| source_sido / source_sigungu | 원천 시도·시군구, nullable |
| source_library_name | 원천 도서관명. 내부 조회는 `library_id`를 사용 |
...
같은 제목이라도 운영 연월일 또는 원천 게시물이 다르면 별도 `Program`으로 저장한다. `ProgramSession`은 사용하지 않는다. `application_status`와 `operation_status`는 현재 날짜를 기준으로 재계산해 필요한 행만 갱신하는 캐시성 상태이며, 별도 `*_raw` 또는 `effective_*` 필드를 두지 않는다. 목록 조회에서는 상태 기반 필터를 적용하기 전에 노출 후보의 상태를 먼저 갱신하고, 상세 조회에서는 대상 프로그램의 상태를 응답 직전에 갱신한다.

상태 계산 규칙:

- `application_required=False` → `신청없음`
- `application_required=True`이고 오늘이 신청 시작일 전 → `NULL`
- 신청 시작일 이상·종료일 이하 → `신청가능`
- 오늘이 신청 종료일 후 → `신청마감`
- 신청 필요 여부 또는 신청 날짜가 부족하거나 날짜 범위가 잘못됨 → `NULL`
- 오늘이 운영 시작일 전 → `예정`
- 운영 시작일 이상·종료일 이하 → `진행중`
- 오늘이 운영 종료일 후 → `종료`
- 운영 날짜가 부족하거나 범위가 잘못됨 → `NULL`

`신청마감`은 정원 마감이 아니라 신청 가능 기간이 끝났다는 의미다.
...
| 시설 | `has_children_room=True` | `children_room`, `children_friendly` |
| 시설 | `has_cafe=True`, `has_lounge=True` | `cafe`, `stay_friendly` |
...
- `parking`, `wifi`, `children_room`, `accessible_facility` 같은 객관형 필터는 `LibraryFacilityProfile`의 명시적 `True` 또는 `field_rule|facility_rule|manual` 직접 근거의 `LibraryTag`만 사용한다.
...
| Program | `UNIQUE(provider_code, external_program_key)`, `(library_id, operation_start_date)`, `(is_visible, operation_status, operation_end_date)`, `(application_status, application_end_date)` |
...
| SourceSyncRun | `(source_name, -started_at)`, `(status, -started_at)`, target_month 1~12 check |
