...
10. `오늘의 추천 도서관`은 날짜별 기준으로 생성한 공통 기본 추천 3개다. 같은 날짜·지역·알고리즘 버전에는 모든 사용자에게 같은 순서로 제공하며 수동·행동 선호로 재정렬하지 않는다.
...
|---|---|---|---|
| 홈 | 도서관, 통계, 태그, 후기 rollup, 추천 규칙 | 오늘의 추천 도서관 3개, 회원용 여기는 어때요? 최대 3개, 6개 테마별 추천 | 순서: 오늘의 추천 → 여기는 어때요? → 테마별 추천; 기본 추천은 개인화하지 않음 |
...
- 같은 `Tag` 행을 공식 필드 기반 연결과 사용자 후기 연결이 공유할 수 있다. 예를 들어 `parking`은 `LibraryFacility`에서 직접 생성될 수도 있고 후기에서 선택될 수도 있다.
...
`program_rollup`은 해당 도서관의 프로그램 태그, `review_rollup`은 공개 후기의 사용자 선택 태그를 집계한 결과다. 후기 본문 자동 태깅은 사용하지 않는다. `review_rollup`은 주관적 경험 신호이므로 `field_rule`, `facility_rule`, `manual` 근거를 대체하지 않는다. 예를 들어 후기에서 `parking`이 선택되어도 `LibraryFacility.parking=available`을 생성하지 않는다.
...
공개되는 오늘의 추천은 상위 3개이며 사용자별로 순서를 바꾸지 않는다. 날짜 순환으로 예정된 테마에서 근거가 충분한 후보를 3개 이상 만들지 못하면, 활성 테마를 `display_order` 순서로 이어서 확인해 처음으로 조건을 충족하는 테마를 실제 `theme_id`로 저장한다. 모든 테마가 기준을 충족하지 못하면 결측이 비교적 적은 `rich_collection`을 최종 fallback으로 사용하고 결측 후보는 후순위에 둔다.

#### 홈 개인화 결과의 비영속 정책

`여기는 어때요?` 결과를 저장하는 별도 테이블은 두지 않는다. 요청 시 다음 방식으로 최대 3개를 계산한다.

1. 오늘의 추천 상위 3개를 후보에서 제외한다.
...
5. 결과가 부족하면 같은 선호 지역의 활성 도서관과 데이터 충실도가 높은 후보로 채우되, 오늘의 추천과 중복시키지 않는다.
...
- `parking`, `wifi`, `children_room`, `accessible_facility` 같은 객관형 필터는 `LibraryFacility` 또는 직접 근거의 `LibraryTag`를 사용한다.
...
| 오늘의 추천 도서관 세트 | 매일 | 최근 90일 | 추천 날짜·subtitle·공통 3개 |
...
- 홈의 오늘의 추천 도서관 3개, 데이터 있는 회원의 여기는 어때요? 최대 3개, 하단 6개 테마별 추천
