...
- 문서 버전: 2.3
- 작성 기준일: 2026-06-23
- 기준 자료: 서비스 목업, 메인페이지 서술, 업데이트된 데이터셋 명세, 전국도서관표준데이터, 도서관 정보나루 Open API, 한국천문연구원 특일 정보 API
...
19. 시설 JSON의 한 행은 `LibraryFacilityProfile` 한 행과 대응한다. 필드가 명시적으로 `true/false`이면 `True/False`, 필드 누락은 `NULL`, 도서관 자체의 시설 행 부재는 프로필 부재로 표현한다.
20. 외관 이미지와 시설 데이터의 미수집은 정상 상태다. 미수집을 이미지 부재·시설 부재라는 사실로 오해하지 않는다.
...
22. 당일 홈 추천은 `open`으로 확정된 도서관만 사용한다. 후보 부족 시 결과 수를 줄일 수 있으나 `closed` 또는 `unknown`을 포함하지 않는다.
23. 현재 필수 정보나루 범위에 없는 `itemSrch`, `bookExist`, `loanItemSrchByLib`, `libSrch`를 전제로 하는 모델·기능은 MVP에서 만들지 않는다. 소장 도서관은 `libSrchByBook`, 인기 도서는 `loanItemSrch`의 전국·지역 조건으로 제공한다.
...
| `libraries` | `Library`, `LibraryExternalIdentifier`, `LibraryOpeningHour`, `LibraryClosureRule`, `PublicHolidayCalendar`, `PublicHoliday`, `LibraryDailySchedule`, `LibraryStatisticSnapshot`, `LibraryFacilityProfile`, `LibraryTag`, `LibraryImage` | 도서관 검색·운영·시설 프로필·태그·이미지 연결 |
...
| 시설 JSON·운영자 보정 | 선택적 1:1 nullable boolean 프로필 | 새 파일 확인 또는 검수 시 idempotent upsert | `LibraryFacilityProfile` |
...
| 공식·기본 이미지 | 실제 이미지가 있는 엔터티만 연결, 나머지는 규칙 기반 대체 | 이용조건·링크 점검 | `MediaAsset`, 연결 모델, `DefaultMediaAssetRule` |
...
- 시설 boolean은 nullable로 저장한다. `False`는 원천 행이 해당 필드를 명시적으로 false로 제공한 경우에만 사용하고, 필드·행 부재는 `NULL` 또는 프로필 부재로 둔다.
...
공휴일 휴관 규칙을 가진 도서관은 해당 연도의 `PublicHolidayCalendar.is_complete=True`일 때만 공휴일 여부를 확정한다. 달력이 불완전하면 그 영향을 받는 날짜의 상태를 `unknown`으로 둔다.
...
- 검색의 긍정 시설 필터는 해당 `has_*` 필드가 `True`인 도서관만 포함한다. `False`, `NULL`, 프로필 부재는 긍정 결과에 포함하지 않는다.
...
- `parking`, `wifi`, `children_room`, `accessible_facility` 같은 객관형 필터는 `LibraryFacilityProfile`의 명시적 `True` 또는 직접 근거의 `LibraryTag`를 사용한다.
...
| LibraryFacilityProfile | `UNIQUE(library_id)` 또는 OneToOne |
...
- `평일운영시작/종료시각`은 월~금에만 매핑하고, `토요일운영시작/종료시각`은 토요일에만 매핑한다. 토요일 시간을 일요일에 복사하지 않는다.
- 표준 데이터에는 일요일 전용 운영시간 필드가 없다. `휴관일`이 일요일 휴관을 명시하면 `closed`, 별도 공식 근거가 없으면 일요일 운영 상태는 `unknown`으로 둔다.
- `공휴일운영시작/종료시각`은 `day_type=public_holiday` 규칙으로 저장하고, 실제 날짜 판정은 완전한 `PublicHolidayCalendar`와 결합한다.
...
현재 프로그램 JSON에는 설명·장소·정원·비용·이미지 필드가 없으므로 해당 값을 추정해 채우지 않는다. 프로그램 카드 이미지는 기본적으로 `category_code`별 `DefaultMediaAssetRule`을 사용하며, 추후 공식·관리자 이미지가 있을 때만 `ProgramImage`를 생성한다.
...
`library_name`, `sigungu`로 `Library`를 매칭한다. 매칭되는 시설 행이 없는 도서관은 정상이며 프로필을 생성하지 않는다. 행이 존재하는 경우 명시적 boolean은 그대로 저장하고 필드 누락만 `NULL`로 둔다. 원천 명세의 `has_cafe` 비고가 `건물`로 적힌 부분은 필드명·예시값과 모순되는 문서 오기로 간주하되, 실제 JSON 값과 수집 근거를 확인하기 전에는 다른 의미로 재해석하지 않고 `has_cafe` 코드 그대로 보존한다.

### 이미지 데이터

| 원천 필드 | 저장 위치 |
|---|---|
| `library_name`, `sigungu` | `Library` 매칭 보조 |
| `image_url`, `image_url_2` | 각각 `MediaAsset.original_url` + `LibraryImage` |
| `license_type` | `MediaAsset.license_code` |

외관 이미지 데이터에 행이 없는 도서관과 URL이 하나만 있는 행을 정상적인 가능성으로 처리한다. 매칭 실패 행은 reject report 대상으로 두며 이미지 행만으로 새 `Library`를 만들지 않는다. 빈 URL이나 같은 URL의 중복은 `MediaAsset`으로 만들지 않는다. `license_type`은 원문 또는 검증된 정규화 코드로 보존하되, 데이터셋에 없는 출처 기관명·출처 페이지·세부 허용 범위를 임의로 생성하지 않는다. 실제 `LibraryImage`가 없으면 `library_type`별 `DefaultMediaAssetRule`을 사용하고, 유형별 규칙도 없으면 `library/default`를 사용한다.
...
| 프로그램 강좌 데이터 | 선택적 | 해당 도서관에 수집된 프로그램이 없음 | 빈 프로그램 목록 허용 |
| 도서관 외관 사진 | 선택적이며 많은 도서관이 없을 수 있음 | 공식·수집 이미지 미보유 또는 미수집 | 유형별·공통 대체 이미지 |
| 도서관 시설 데이터 | 선택적이며 많은 도서관이 없을 수 있음 | 시설 부재가 아니라 시설 정보 미확인 | 프로필 부재 또는 nullable 필드; 긍정 필터에서 제외 |
...
| `srchDtlList` | ISBN 기준 책 상세 | `Book` 갱신; `loaninfoYN=N`을 기본으로 사용 |
| `loanItemSrch` | 전국·부산·세부 지역의 인기 대출 도서 | `PopularBookSnapshot`, `PopularBookItem` |
| `recommandList` (`type=reader`) | 최대 5개 ISBN 기반 다음 읽을 책 후보 | 반환 도서를 `Book` upsert, 입력 ISBN 조합별 단기 캐시; 별도 관계 테이블 없음 |
| `libSrchByBook` | 부산·세부 지역의 특정 ISBN 소장 도서관 | `LibraryHolding`; 응답 `libCode`는 `LibraryExternalIdentifier`에 on-demand upsert |

현재 필수 데이터셋에 없는 `libSrch`, `itemSrch`, `bookExist`, `loanItemSrchByLib`를 MVP 구현 전제로 두지 않는다.

주의사항:

- `srchBooks.loan_count`는 검색 응답의 시점값이므로 `Book` 현재값으로 영구 저장하지 않는다.
- `loanItemSrch`의 `region`, `dtl_region`으로 전국·부산·구 단위 인기 목록을 만들며 특정 도서관 범위는 제공하지 않는다.
- 인기 목록에는 집계 범위·기간·필터를 함께 제공한다.
- `libSrchByBook`의 `closed`, `operatingTime`은 매칭 보조·원문 표시용이며 내부 `LibraryDailySchedule`을 덮어쓰지 않는다.
- `fax`처럼 서비스에서 사용하지 않는 응답 필드는 저장하지 않을 수 있으나 normalizer contract에서 명시적으로 무시한다.
...
- 정보나루 도서 검색·상세·소장 도서관·다독자 기반 다음 책 추천
...
- 실시간 열람실 잔여 좌석·방문자 수
- 전국 전체 장서 사전 적재
- 정보나루 `itemSrch` 복본·청구기호, `bookExist` 대출 가능 여부, `loanItemSrchByLib` 특정 도서관 인기 목록
