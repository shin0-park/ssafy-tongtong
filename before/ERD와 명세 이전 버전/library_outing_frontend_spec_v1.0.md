...
| F1302 API 활용 | 책 검색·상세·인기·소장 도서관, 위치 지도, 백엔드 경유 외부 API |
| F1303 커뮤니티 | 후기 목록·상세·작성·수정·삭제·좋아요 |
| F1304 RESTful 원칙 | 자원별 GET·POST·PATCH·PUT·DELETE 계약 준수 |
| NF1302 API Key 관리 | Vue에서 외부 API Key를 사용하거나 노출하지 않음 |
...
| 후기 작성자 | 특정 후기의 수정·삭제 권한을 가진 회원 |
| 관리자 | 백엔드 관리 대상이며 v1.0 SPA 관리자 UI 범위 밖 |

### 2.2 기능 권한표

| 기능 | 비회원 | 회원 | 작성자 조건 |
|---|---:|---:|---:|
| 홈·도서관·책·프로그램·후기 조회 | 가능 | 가능 | 해당 없음 |
| 위치 기반 가까운 도서관 | 위치 동의 시 가능 | 위치 동의 시 가능 | 해당 없음 |
| 도서관·책·프로그램 저장 | 로그인 유도 | 가능 | 해당 없음 |
| 후기 좋아요 | 로그인 유도 | 가능 | 해당 없음 |
| 후기 작성 | 로그인 유도 | 가능 | 해당 없음 |
| 후기 수정·삭제 | 불가 | 본인 후기만 | 필요 |
| 나의 나들이 | 로그인 유도 | 가능 | 해당 없음 |
| 프로필·선호 설정 | 로그인 유도 | 가능 | 해당 없음 |
| 개인화 추천 | 미노출 | 유효 신호가 있을 때 노출 | 해당 없음 |

### 2.3 인증이 필요한 행동의 처리

비회원이 저장·좋아요·후기 작성·나의 나들이에 접근하면 현재 맥락을 잃지 않고 로그인으로 전환한다.
...
후기 작성은 로그인 후 작성 화면으로만 돌아오며, 작성 초안이 있으면 복원한다.
...
후기 수정·삭제 버튼은 `review.author.id === authStore.user.id`일 때만 표시한다. 다만 UI에서 숨겼더라도 서버의 `403` 응답을 최종 권한 판단으로 처리한다.
...
v1.0은 상단 헤더와 햄버거 드로어를 사용한다.

- 헤더: 로고, 검색 페이지 바로가기 선택 영역, 메뉴 버튼
- 드로어: 데스크톱과 동일한 전체 메뉴
- 드로어가 열리면 배경 스크롤을 잠근다.
- ESC, 바깥 영역 클릭, 닫기 버튼으로 닫는다.
- 열릴 때 첫 메뉴로 Focus를 이동하고 닫힐 때 메뉴 버튼으로 복귀한다.

하단 탭 내비게이션은 v1.0 범위에서 제외한다.
...
`VITE_` 변수는 브라우저 번들에 포함되므로 비밀값을 넣지 않는다.
...
- 인증 어댑터가 제공하는 access token이 있으면 `Authorization: Bearer`를 추가한다.
- `FormData` 요청은 브라우저가 boundary를 생성하도록 `Content-Type`을 직접 지정하지 않는다.
- 요청 ID를 프론트 로그에 남길 수 있도록 로컬 식별자를 생성할 수 있다.

응답 Interceptor:

- 공통 오류를 `NormalizedApiError`로 변환한다.
- `401`이면 인증 갱신을 한 번만 시도하고 동시 요청을 대기시킨다.
- 갱신 실패 시 인증 상태를 초기화하고 현재 Route를 redirect로 보존한다.
- `429`는 `Retry-After`를 읽어 재시도 가능 시점을 안내한다.
- 자동 재시도는 GET의 일시적 네트워크 오류에 한해 최대 1회 허용한다.
- POST·PATCH·DELETE는 자동 재시도하지 않는다.
...
기본 권장 방식은 짧은 수명의 access token과 HttpOnly refresh cookie다. 백엔드가 세션 인증을 선택하면 `authService` 내부만 교체하고 Store와 페이지 계약은 유지한다.
...
- `requiresAuth`: 비회원 접근 시 로그인으로 이동
...
`redirect`는 내부 경로만 허용한다. `http://`, `//`처럼 외부 이동이 가능한 값은 무시한다.
...
기준정보는 세션 동안 재사용하며 서버 오류 시 이미 받은 데이터가 있으면 유지한다.
...
|---|---|---|
| 지역·목적·태그 기준정보 | 세션 | 로그아웃 시 개인 기준정보만 정리, 앱 새 버전 |
...
프론트는 직접 색상값을 컴포넌트에 반복 작성하지 않고 CSS 변수로 관리한다.

```css
:root {
  --color-primary: var(--bs-primary);
  --color-primary-emphasis: var(--bs-primary-text-emphasis);
  --color-surface: #fff;
  --color-surface-subtle: var(--bs-tertiary-bg);
  ...
  --radius-md: 0.75rem;
  --radius-lg: 1rem;
  --shadow-card: 0 0.25rem 1rem rgb(0 0 0 / 8%);
}
```

브랜드 색상 확정 전에는 Bootstrap 기본값을 사용하되 대비 기준을 충족한다.
...
| API | `POST /auth/login/` |
| Layout | `AuthLayout` |
| 성공 이동 | 유효한 `redirect` → pending intent 대상 → 홈 순서 |
...
로그인 성공
→ authStore.user 갱신
→ reference/profile 캐시 필요 시 초기화
→ pending intent 조회
→ save/like intent이면 원 Route에서 한 번 재실행
→ redirect 또는 홈 이동
```
...
3. 비밀번호
4. 비밀번호 확인
5. 필수 약관 동의 영역 — 실제 약관 문서가 없는 경우 v1.0에서는 서비스 이용 동의 Checkbox만 제공하거나 제외 여부를 팀이 확정
6. 가입 버튼
7. 로그인 링크
...
필터는 모바일에서 바텀시트 또는 전체 높이 Drawer로 제공한다. 적용 버튼을 누르기 전까지 Route Query를 변경하지 않는 임시 상태를 사용한다.
...
- 좌표가 있으면 Map 표시
- 좌표가 없으면 주소 텍스트만 제공
- 지도 SDK Key가 필요하면 공개 가능한 도메인 제한 Key만 사용하며, 유료·비밀 API Key는 사용하지 않음
- 지도 로드 실패 시 주소와 외부 지도 검색 CTA를 MAY 제공
- 지도는 키보드 사용자를 가두지 않으며 “지도 건너뛰기” 또는 텍스트 대체 제공
...
- 메모 필드는 API에 존재하지만 v1.0 UI에서는 노출하지 않는다.
...
| 전송 | v1.0 기본은 multipart FormData |
...
배열 직렬화 형식은 백엔드 parser와 구현 전에 확정한다. JSON 문자열 또는 반복 field 중 하나만 사용한다.
...
- 기존 데이터를 Form 초기값으로 사용
- 기존 이미지 삭제·신규 이미지 추가 계약은 TBD이며 구현 전 백엔드와 확정
- 도서관 변경 허용 여부는 v1.0에서 **불허 권장**; 백엔드가 허용하더라도 후기의 대상 일관성을 위해 read-only로 표시
- 태그·관련 책·프로그램은 전체 교체
- 403이면 `/403` 또는 상세로 이동하고 권한 안내
...
- 공개·숨김 상태 표시 계약은 백엔드 응답에 상태 field가 포함될 때만 제공
- 수정·삭제 제공
- 삭제 후 목록 count와 대시보드 count 갱신
- hidden 후기 포함 여부는 백엔드 권한 정책에 따름
...
- 지도 내용의 텍스트 대체로 주소·좌표·외부 지도 링크 제공
- 성향 Chart는 축 이름과 비율 목록을 함께 제공
- 색상만으로 축을 구분하지 않음
...
- Vue는 정보나루, GMS, 공공데이터포털 API를 직접 호출하지 않는다.
- 브라우저 Network에 외부 API 인증키가 나타나면 결함으로 처리한다.
- `.env`의 `VITE_*`는 공개값만 사용한다.
- GMS 호출이 필요한 경우 `Vue → Django → GMS → Django → Vue` 흐름만 사용한다.

### 22.2 인증 정보

- access token은 가능한 한 메모리에 유지한다.
- refresh token은 HttpOnly Secure SameSite cookie를 권장한다.
- 구현상 token을 Web Storage에 저장해야 하면 위험을 문서화하고 XSS 방어를 강화한다.
- 로그아웃 시 사용자 Store, pending intent, 개인 캐시, 작성자 전용 데이터 제거
- 공개 기준정보와 일반 검색 Query는 유지 가능
...
- 기준정보 병렬 조회
- 상세과 관련 preview가 한 응답에 있으면 중복 API 호출 금지
- 리뷰 상세을 hover prefetch하지 않음 — 조회수 증가 부작용 방지
- 목록 재조회 중 기존 결과 유지

### 23.4 렌더링

- 카드 수가 서버 페이지 크기 20 수준이므로 v1.0에서 Virtual Scroll 불필요
- Chart library는 필요 화면에서만 lazy import
- 지도 SDK는 지도 영역이 viewport에 가까워졌을 때 로드 SHOULD
...
| P1 | 출처 오버레이, 접근성·반응형 완성, 캐시 무효화 정교화 |
| P2 | Undo, Share, 고급 Chart, 지도 지연 로드, 추가 애니메이션 |
...
- 키보드로 핵심 흐름 완료 가능
- Console error 없음
- API Key와 민감정보 번들·Network·로그 비노출
- Production build 성공
- 핵심 Unit·E2E Test 통과
...
다음 항목은 구현 전에 백엔드 또는 팀 합의가 필요하다.

1. 로그인 응답과 token 갱신 endpoint·cookie 정책
2. 회원가입 성공 시 자동 로그인 여부
3. 프로필 응답의 정확한 count·이미지 field 구조
4. 후기 multipart 배열 직렬화 방식
5. 후기 수정 시 기존 이미지 삭제·순서 변경 API
6. 내가 쓴 후기 목록에 hidden 후기 포함 여부와 표시 field
...
10. 지도 Provider와 공개 Key의 도메인 제한 정책
11. 저장 memo를 v1.1에서 UI에 노출할지
12. 프로필 이미지 삭제 방식
13. Backend pagination의 `page_size` 최대값
14. Auth refresh 실패 후 보류 멱등 행동의 유효시간

미확정 항목은 `TBD` 상태로 Issue에 등록하고, 구현자가 독자적인 응답 형태를 만들지 않는다.
...
|---|---|---|---|
| 로그인 | POST | `/auth/login/` | 인증 |
| 회원가입 | POST | `/auth/signup/` | 계정 생성 |
| 공통 | POST | `/auth/logout/` | 로그아웃 |
...
| `page` | integer | 1 | 페이지 |
| `page_size` | integer | 20 | v1.0 UI에서 기본값 유지 |
...
- `sessionStorage`에 하나만 저장
- 30분 경과 시 자동 폐기
- 성공적으로 소비하면 즉시 삭제
- 같은 자원 상태가 이미 목표 상태이면 추가 요청 없이 완료
- 안전한 내부 redirect만 허용
- 사용자가 로그인 화면에서 취소해도 원래 공개 화면으로 돌아갈 수 있음
