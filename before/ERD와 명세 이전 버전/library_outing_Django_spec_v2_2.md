...
- 문서 버전: 2.2
- 작성 기준일: 2026-06-22
- 기준 ERD: `library_outing_ERD_v2_2.md`
...
1. 모든 사용자에게 동일하게 제공되는 **오늘의 추천 도서관** 3개
2. 개인화 입력이 있는 회원에게만 제공되는 **여기는 어때요?** 최대 3개
3. 사용자가 직접 선택하는 6개 **테마별 추천**
4. 도서관 검색·상세·일자별 운영 여부 조회
5. 도서 검색과 소장 도서관·대출 가능 여부 조회
6. 전국·부산·특정 도서관 인기 대출 도서 목록
...
13. 공휴일·도서관·프로그램·인기 도서 데이터의 예약 갱신과 파생 데이터 생성
...
- 오늘의 추천 도서관 3개와 테마별 추천은 공통 결과이며 개인화로 재정렬하지 않는다.
...
- `nearby`, `open_now`, `loan_available`, `current_popular`, `low_occupancy`, `not_too_crowded` 같은 동적·시점 의존 상태 태그 seed를 금지한다.
...
- 오늘의 추천 공개 결과는 상위 3개이며 사용자별로 재정렬하지 않음
...
| 시설 available | parking/wifi/accessibility | `parking`, `wifi`, `accessible_facility` |
...
- `parking`, `wifi`, `children_room`, `accessible_facility` 같은 객관형 필터는 `LibraryFacility` 또는 직접 근거(`field_rule|facility_rule|manual`)만 사용한다.
...
2. 없으면 오늘 예정 테마로 동기 최소 계산한다. 이 계산도 실패할 때만 최근의 동일 테마 성공 세트를 `is_stale=true`로 반환한다.
3. 공통 기본 순위 상위 3개를 `today`에 그대로 반환한다. 사용자 선호로 순서를 바꾸지 않는다.
4. 인증 사용자가 활성 `UserPreferredTag`, `UserPreferredRegion` 또는 1건 이상의 유효 행동 신호를 가지고 있으면 `for_you.visible=true`로 판정한다.
5. `for_you`는 오늘의 추천 3개를 후보에서 제외하고 수동 선호와 임시·정식 행동 성향을 반영해 최대 3개를 계산한다.
6. `themes`에는 활성 `Purpose` 6개를 `display_order` 순서로 반환한다. 실제 테마 결과는 별도 endpoint에서 조회한다.
...
|---|---|---:|---|---|
| 오늘의 추천 도서관 | 모든 사용자 | 3 | 적용하지 않음 | 공통 기본 순위 유지 |
...
무작위로 테마를 바꾸지 않는다. 생성기는 예정 테마부터 활성 테마를 순서대로 순환하며 `DAILY_RECOMMENDATION_MIN_RESULT_COUNT`개 이상의 근거 있는 후보를 만들 수 있는 첫 테마를 실제 테마로 선택한다. 모든 테마가 기준을 충족하지 못하면 `rich_collection`을 최종 fallback으로 사용하고 결측 후보는 후순위에 둔다. 실제 선택된 테마는 `DailyLibraryRecommendationSet.theme`에 저장하므로 같은 날짜·지역의 재요청은 동일한 테마와 subtitle을 사용한다. 지역별 기본 후보를 최대 `DAILY_RECOMMENDATION_CANDIDATE_LIMIT`만큼 미리 저장하고, 공개 응답은 기본 순위 상위 `TODAY_RECOMMENDATION_LIMIT=3`개다.
...
후보군은 공통 후보군에서 오늘의 추천 3개를 제외한 도서관이다.
...
`collecting` 신뢰도는 `PERSONALIZATION_COLLECTING_MAX_CONFIDENCE`를 넘지 않는다. 결과는 최대 `PERSONALIZED_HOME_RECOMMENDATION_LIMIT=3`개이며 오늘의 추천과 중복되면 안 된다. 일치 후보가 부족하면 선호 지역·데이터 충실도 기준으로 채울 수 있지만, 중복 제거 원칙은 유지한다.
...
- 오늘의 추천 도서관: 지역별 매일 생성, 기본 후보 20개 내외, 공개 3개는 공통 순위 유지
- 여기는 어때요?: 요청 시 계산, 사용자별 단기 캐시 가능, 영속 결과 테이블 없음
...
| `recommendations.generate_daily_sets` | 매일 00:10 | 지역별 오늘의 추천 기본 후보 생성 |
...
- 객관형 필터(도서관 유형, 운영, 주차, 와이파이, 어린이자료실, 이동약자 편의 등): `field_rule|facility_rule|manual` 또는 원본 `LibraryFacility`만 사용
...
후기 선택 태그를 근거로 공식 시설 상태를 `available`로 승격하지 않는다.
...
- review rollup이 `LibraryFacility` 상태를 변경하지 않음
- 이미지 최대 수
- 최신순 기본 정렬
- 자신의 후기 저장 거절
- hidden 후기 비노출
...
- 같은 날짜·지역의 오늘의 추천 3개가 사용자와 무관하게 동일한지 확인
...
- 오늘의 추천 세트와 공통 3개, 후보 부족 시 결정론적 테마 fallback
- 홈 섹션 순서와 통합 API
- 테마별 추천 endpoint
- 개인화 없는 상태의 재현성 테스트
...
- 같은 날짜·지역에 모든 사용자에게 동일한 오늘의 추천 도서관 3개
