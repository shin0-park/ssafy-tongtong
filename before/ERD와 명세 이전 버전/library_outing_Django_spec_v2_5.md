...
- 문서 버전: 2.5
- 작성 기준일: 2026-06-23
- 기준 ERD: `library_outing_ERD_v2_5.md`
...
- `application_status`와 `operation_status`는 고정 원천값이나 `*_raw`/`effective_*` 이중 필드가 아니라, 목록·상세 조회 전에 날짜를 기준으로 재계산해 DB에 갱신하는 캐시성 상태다. `신청마감`은 정원 마감이 아니라 신청 기간 종료를 뜻한다.
...
- 시설 데이터는 `LibraryFacilityProfile` 선택적 OneToOne과 nullable boolean 필드로 저장한다. 유효한 JSON 배열/JSONL로 정규화한 뒤 `True`, `False`, `NULL`을 그대로 적재하며, 긍정 시설 필터는 해당 필드가 명시적으로 `True`일 때만 통과시킨다. 파일 완성도와 미매칭은 행 상태값이 아니라 `SourceSyncRun`과 report에서 관리한다.
- 실제 외관 이미지 파일은 CSV다. 220개 행이 있어도 URL이 있는 도서관만 실제 이미지를 보유하며, 공개 사용 조건이 불명확한 asset은 비활성으로 두고 유형별 대체 이미지를 사용한다.
...
- 외부 API·파일 수집 실행은 `integrations.SourceSyncRun`에 `success|failed|partial`, 대상 연·월, 처리 건수와 대표 오류를 기록한다. 도메인 행에는 `draft`, `verified`, `is_verified`, `verified_at` 같은 검수 상태를 두지 않는다.
...
| `integrations` | 외부 API client, provider adapter, JSON/file loader, normalizer, 수집 실행 로그 |

`integrations`는 영속 모델로 `SourceSyncRun`만 소유한다. 별도 `sources` 앱과 `DataSource`, `LibrarySourceRecord`는 만들지 않는다.
...
정규화된 시설 파일의 고정 nullable boolean 필드와 일치하는 선택적 OneToOne 모델이다.
...
- 긍정 시설 필터는 해당 필드가 `True`인 경우만 통과시킴
- `False`, `None`, profile 부재를 모두 같은 부재 사실로 표시하지 않음
- 이름·지역 보정, 중복·미매칭은 profile 필드가 아니라 `SourceSyncRun` 처리 건수와 correction/reject report에 기록
...
- unique `(library, book)`
- `provider_code`, `first_seen_at`, `last_fetched_at`, `is_active`
...
- `library`: 개최 도서관 FK
- `source_sido`, `source_sigungu`, `source_library_name`: 원천 지역·도서관명 보존
...
- 분류 코드:
  ...
  - `exhibition`
  - `other`
- `target_text`, `target_codes`: 원문과 정규화 JSON list
- `application_required`: nullable boolean
- `application_start_date`, `application_end_date`: nullable date
- `application_status`: `신청가능|신청마감|신청없음|null`
- `operation_start_date`, `operation_end_date`: nullable date
- `operation_status`: `예정|진행중|종료|null`
- `source_board`, `source_url`, `post_date`, `collected_at`
...
`ProgramSession` 모델은 만들지 않는다. 같은 제목이 반복되어도 운영기간·게시물이 다르면 별도 `Program`이다. 현재 프로그램 JSON에는 설명·장소·정원·비용·이미지 필드가 없으므로 모델·import에서 값을 추정하지 않는다.

`application_status`와 `operation_status`는 원천 고정값이나 별도 `*_raw`/`effective_*` 필드가 아니라 조회 시점 기준으로 재계산해 저장하는 캐시성 상태다. 권장 service는 다음 규칙을 적용한다.

```text
application_required is False
→ 신청없음

application_required is True and today < application_start_date
→ null

application_start_date <= today <= application_end_date
→ 신청가능

today > application_end_date
→ 신청마감

신청 필요 여부·날짜 부족 또는 역전
→ null


today < operation_start_date
→ 예정

operation_start_date <= today <= operation_end_date
→ 진행중

today > operation_end_date
→ 종료
...
`신청마감`은 정원 마감이 아니라 신청 가능 기간 종료를 뜻한다. 프로그램 목록 API는 `application_status`·`operation_status` 필터를 적용하기 전에 활성 노출 후보를 대상으로 `update_program_statuses(today, base_queryset)`를 호출하고, 상세 API는 대상 행을 응답하기 전에 갱신한다. 달라진 행만 `bulk_update`한다.
...
외부 API 또는 파일 import 한 번의 실행 요약을 저장한다. 도메인 행의 검수 상태를 나타내지 않는다.
...
```python
models.CheckConstraint(
    condition=Q(target_month__isnull=True) | Q(target_month__range=(1, 12)),
    name="ck_source_sync_target_month",
)
models.Index(fields=["source_name", "-started_at"])
models.Index(fields=["status", "-started_at"])
```

실행 시작 시 `status=NULL`인 row를 만들고, 종료 시 `success|failed|partial` 중 하나와 finished_at·처리 건수를 갱신한다. 상세 reject·warning·unmatched 목록은 report 파일 또는 구조화 로그에 두고 `error_message`에는 대표 오류만 저장한다. 공휴일 수집은 연간 실행 1건 또는 월별 실행 12건 중 하나를 택할 수 있으나, `PublicHolidayCalendar.synced_month_count`와 대조 가능해야 한다.
...
- `parking`, `wifi`, `children_room`, `accessible_facility` 같은 객관형 필터는 `LibraryFacilityProfile`의 명시적 `True` 또는 직접 근거(`field_rule|facility_rule|manual`)만 사용한다.
...
목록 카드의 `facilities`에는 `LibraryFacilityProfile`에서 값이 `True`인 코드만 넣는다. `False`, `NULL`, profile 부재를 동일한 “없음”으로 단정하지 않는다. 상세 응답은 `facility_coverage=complete|partial|missing`과 11개 nullable boolean을 함께 제공할 수 있다.
...
목록 조회는 상태 필터를 적용하기 전에 활성 노출 후보에 `update_program_statuses(timezone.localdate(), base_queryset)`를 실행한 뒤 최종 queryset을 구성한다. 상세 조회는 대상 프로그램의 상태를 응답 전에 갱신한다. 프로그램 상세는 개최 도서관, 신청 필요 여부·신청 기간·현재 신청 상태, 운영 기간·현재 운영 상태, 태그, 실제 또는 대체 이미지, 공식 원문 URL, 수집 시각을 반환한다. 회차 배열과 내부 신청 이력은 반환하지 않는다.
...
- `LibraryFacilityProfile`의 명시적 `True` 필드
- 활성 `LibraryTag`
...
- 목록 조회에서는 상태 기반 django-filter를 적용하기 전에 활성 노출 후보의 현재 날짜와 신청·운영 기간을 비교한다. 상세 조회에서는 대상 행을 응답 전에 비교한다.
- 값이 달라진 행만 `bulk_update`하여 `application_status`, `operation_status`, `updated_at`을 갱신한 뒤 최종 필터·정렬·직렬화를 수행한다.
- 동일 날짜의 반복 조회에서는 쓰기 횟수를 최소화한다.
- 날짜가 부족하거나 역전된 행은 해당 상태를 `NULL`로 두고 warning log를 남긴다.
- 별도 `*_raw`, `effective_*`, 상태 이력 테이블은 만들지 않는다.
...
| `libraries.import_facility_dataset` | 정규화 파일·수동 | 시설 프로필 nullable boolean upsert; 미매칭·중복 report |
...
모든 외부 API·파일 수집 task는 실행 시작 시 `SourceSyncRun`을 만들고 종료 시 status·finished_at·처리 건수를 갱신한다. task ID, provider code, 입력 파일 checksum과 상세 실패 목록은 구조화 로그·report 파일에 함께 기록한다.
...
correction 적용도 audit report에 남긴다. import 실행 요약은 `SourceSyncRun`에 기록하고, correction·reject의 상세 목록은 report 파일에 둔다.
...
2. 분류·대상 코드 정규화
3. `application_required`, 신청·운영 날짜와 초기 상태 파싱
4. 외부 key 생성
5. Program upsert
6. 현재 날짜 기준 `application_status`, `operation_status` 계산
7. 이번 입력에 없는 기존 provider 행의 soft delete 정책 적용
8. ProgramTag 재계산
9. 개최 도서관 program rollup 재계산

같은 제목이라도 날짜가 다르면 하나로 병합하지 않는다.

원천 `application_status`, `operation_status`는 같은 이름의 캐시성 상태 필드에 초기 적재하고, 신청·운영 날짜를 기준으로 즉시 보정한다. 이후 목록·상세 조회에서도 다시 계산한다. `신청마감`은 신청 기간 종료를 의미하며, 별도 `*_raw`, `effective_*`, 신청 상태 이력 테이블은 만들지 않는다. 원천에 없는 설명·장소·정원·비용·이미지는 생성하지 않는다.
...
시설 필터는 해당 nullable boolean이 `True`인 경우만 통과시킨다. `False`, `NULL`, profile 부재를 서로 같은 “시설 없음”으로 표현하지 않는다.
...
- 객관형 필터(도서관 유형, 운영, 주차, 와이파이, 어린이자료실, 이동약자 편의 등): `field_rule|facility_rule|manual` 또는 `LibraryFacilityProfile`의 명시적 `True`만 사용
...
- 시설 프로필 부재·nullable 필드(`True|False|NULL`) 필터
...
- `SourceSyncRun`을 source·status·대상 연·월·시작 시각으로 필터
- 처리 건수와 대표 오류 확인
- 도메인 행의 검수 상태를 변경하는 action은 제공하지 않음
...
- 시설 값 `True`만 긍정 필터 통과
- `False`, `None`, profile 부재는 긍정 필터 불통과이며 서로 같은 부재 사실로 직렬화하지 않음
...
- 분류·대상 태그 생성
- 신청 불필요 → `신청없음`
- 신청 시작 전 → `application_status=None`
- 신청 기간 내 → `신청가능`
- 신청 종료일 후 → `신청마감`이며 인원 마감 의미로 해석하지 않음
- 운영 시작 전·기간 내·종료 후 → `예정|진행중|종료`
- 신청·운영 날짜 부족 또는 역전 → 상태 `None`
- 목록·상세 조회 시 오래된 상태가 필요한 행만 갱신됨
- `application_status_raw`, `operation_status_raw`, `effective_*` 필드 없음
- 분류별 fallback 이미지
...
- API/file 실행 시작·성공·실패·부분 성공에 따른 `SourceSyncRun` 상태와 처리 건수
...
- 잘못된 날짜와 필드별 0값 정책
- 시설 중복·지역 불일치·이름 오타·미매칭 처리 건수와 report
...
- import reject count
- SourceSyncRun success/failed/partial 비율과 처리 건수
- 운영표 unknown 비율
- 도서관 이미지 fallback 비율
...
- SourceSyncRun 모델·실행 기록 service
- 공통 예외·API schema
...
- 검색·상세
- 이미지 CSV import
- 시설 파일 정규화·nullable boolean import·미매칭 report
...
- JSON import
- 신청 필요 여부·신청 기간·운영 기간 필드
- 조회 시 `application_status`, `operation_status` 재계산·갱신
- ProgramTag
...
- 분류·대상·도서관 검색
- 신청 필요 여부·신청 기간·현재 신청 상태
- 운영 기간·현재 운영 상태
- 조회 시 날짜 기준 상태 갱신; `신청마감`은 신청 기간 종료
...
- Celery 주기 작업
- SourceSyncRun 실행 이력과 reject·warning report
...
1. 시설 작업 파일을 JSON 배열 또는 JSONL로 먼저 정규화하고, `true/false/null`은 `True/False/None`으로 그대로 저장하며 행 부재는 profile 부재로 유지하는지 확인한다. 파일 완성도와 미매칭은 `SourceSyncRun`·report로 관리하고 profile에 검수 상태를 두지 않는다.
2. 프로그램 제공 파일에 안정적인 원천 ID가 있는지 확인하고 없으면 hash 구성 필드를 확정한다. `application_status`와 `operation_status`의 조회 시 갱신 service가 날짜 경계와 `NULL` 규칙을 따르는지 함께 확인한다.
