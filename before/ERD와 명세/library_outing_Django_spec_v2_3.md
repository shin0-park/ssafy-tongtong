...
- 문서 버전: 2.3
- 작성 기준일: 2026-06-23
- 기준 ERD: `library_outing_ERD_v2_3.md`
...
- 시설 데이터는 `LibraryFacilityProfile` 선택적 OneToOne과 nullable boolean 필드로 저장한다. 시설 행 부재는 시설 부재가 아니라 미확인이다.
- 도서관 외관 이미지와 시설 데이터는 부분 커버리지 데이터다. 실제 이미지가 없으면 유형별 대체 이미지를 사용하고, 시설 프로필이 없으면 긍정 시설 필터에 포함하지 않는다.
...
- 현재 필수 정보나루 API는 `srchBooks`, `srchDtlList`, `loanItemSrch`, `recommandList`, `libSrchByBook`이다. 이 범위에 없는 복본·청구기호, 대출 가능 여부, 특정 도서관 인기 목록은 MVP에서 구현하지 않는다.
...
| `libraries` | 도서관·운영시간·휴관·공휴일 달력·운영표·통계·시설 프로필·도서관 태그·이미지 연결 |
...
업데이트된 시설 JSON의 고정 boolean 필드와 일치하는 선택적 OneToOne 모델이다.
...
- 현재 필수 명세에 없는 `libSrch`, `itemSrch`, `bookExist`, `loanItemSrchByLib` 메서드를 기본 interface에 넣지 않는다.
...
- `library_name`, `sigungu`로 도서관을 매칭한다.
- `image_url`, `image_url_2`는 각각 독립적인 `MediaAsset`·`LibraryImage` 후보로 처리한다.
- URL, license code를 검증한다. 빈 URL과 동일 URL 중복은 asset으로 만들지 않는다.
- `license_type`은 원문 또는 검증된 정규화 코드로 보존한다. 데이터셋에 없는 `source_name`, `source_page_url`, 세부 이용허락 boolean은 추정하지 않는다.
- URL 하나만 존재하거나 도서관에 이미지 행이 전혀 없는 경우를 정상 처리한다.
- 매칭 실패 행은 reject report에 남기고 import 전체를 실패시키지 않으며, 이미지 행만으로 새 도서관을 생성하지 않는다.
- 실제 관계가 있을 때만 연결 모델을 생성한다.
- 이미지가 없는 엔터티에는 연결 행 대신 도서관 유형별 기본 규칙, 그다음 도메인 공통 규칙을 사용한다.

### 9.7 필수 데이터셋 계약

| 데이터셋 | 필수 매핑 키 | 영속 모델 | 커버리지·결측 처리 |
|---|---|---|---|
| 전국 도서관 표준 데이터 | 도서관명·주소·지역·좌표 보조 | `Library`, 운영시간, 휴관 규칙, 통계 | 부산 기준 모체. 필드별 빈 문자열·0·`00:00` 정제; 평일=월~금, 토요일=토요일이며 일요일은 별도 근거 없으면 unknown |
| 프로그램 강좌 JSON | `library_name`, `sigungu` | `Program`, `ProgramTag` | 도서관별 행이 없어도 정상; 빈 프로그램 목록 |
| 도서관 외관 사진 JSON | `library_name`, `sigungu` | `MediaAsset`, `LibraryImage` | 부분 커버리지. 행·URL 부재 시 유형별 fallback |
| 도서관 시설 JSON | `library_name`, `sigungu` | `LibraryFacilityProfile` | 부분 커버리지. 행 부재는 profile 미생성, 필드 누락은 `None`; `has_cafe` 비고의 원천 문서 오기 여부는 별도 검수 |
| 한국천문연구원 공휴일 API | `solYear`, `solMonth` | `PublicHolidayCalendar`, `PublicHoliday` | 초기 배포 및 연초 연도별 12개월 완전 수집. 정상 빈 달도 성공 월; 불완전하면 공휴일 의존 운영표 `unknown` |
| 정보나루 책·소장·인기·추천 API | ISBN·지역·도서관 코드 | `Book`, `LibraryHolding`, 인기 snapshot; 추천은 단기 cache | 현재 명세의 5개 API만 지원; 미조회와 부재를 구분 |

import는 데이터셋별 schema를 검증하고, 매칭 실패·결측·중복·변환 실패를 reject report와 구조화 로그로 남긴다. 외관 이미지와 시설 데이터가 없는 도서관을 오류로 처리하지 않는다.
...
`libSrchByBook` 응답으로 소장 도서관을 조회한다. `libCode`는 내부 `Library` 매칭이 성공한 경우 `LibraryExternalIdentifier`로 on-demand upsert한다. 응답의 `closed`, `operatingTime`은 매칭 보조·원문 표시에는 사용할 수 있지만 내부 `LibraryDailySchedule`을 덮어쓰지 않는다.
...
| `libraries.import_facility_dataset` | 새 파일·수동 | 선택적 시설 프로필 nullable boolean upsert |
...
- `평일운영시작/종료시각`은 월~금에만, `토요일운영시작/종료시각`은 토요일에만 확장한다. 토요일 시간을 일요일에 복사하지 않는다.
- 표준 데이터에 일요일 전용 운영시간이 없으므로, 휴관 문구가 일요일 휴관을 명시하면 `closed`, 별도 공식 근거가 없으면 일요일은 `unknown`으로 계산한다.
...
매칭 키는 `library_name`, `sigungu`다. 하나의 매칭 행은 하나의 `LibraryFacilityProfile`과 대응한다.

boolean 변환:

- 명시적 `true` → 해당 `has_* = True`
- 명시적 `false` → 해당 `has_* = False`
- 필드 누락·해석 불가 → 해당 `has_* = None`
- 매칭되는 시설 행 자체가 없음 → profile 미생성
...
매칭 실패·중복 행은 reject report에 남긴다. 원천 명세의 `has_cafe` 설명이 `건물`로 기재된 부분은 필드명·예시와 모순되므로 데이터 사전 검수 항목으로 남긴다. import 코드는 임의로 다른 의미를 부여하지 않고 `has_cafe` 값을 그대로 카페 여부 필드에 적재하되, 실제 수집 근거가 확인되기 전에는 관리자 화면에 원천 검수 필요 상태를 표시할 수 있다. import 뒤 `True`인 필드만 facility rule 기반 `LibraryTag`의 직접 근거로 사용해 재계산한다.
...
- 객관형 필터(도서관 유형, 운영, 주차, 와이파이, 어린이자료실, 이동약자 편의 등): `field_rule|facility_rule|manual` 또는 원본 `LibraryFacilityProfile`의 명시적 `True`만 사용
...
- 운영시간·휴관·통계·시설 프로필·태그·이미지 inline
- 시설 프로필 부재·nullable 필드 필터
- 운영표 재생성 action
- 태그 재계산 action
...
- 도서관 이미지 행 부재 시 유형별 fallback
- 유형 규칙 부재 시 도메인 default fallback
- URL 하나만 있는 이미지 행 처리
- 없는 rule이면 null
- fallback이 연결 모델로 저장되지 않음
- 라이선스·attribution 응답
- 시설 행 부재 시 profile 미생성
- 시설 필드 `true/false/null`의 구분
- 긍정 시설 필터가 `True`만 포함하고 `False/None/profile 부재`를 제외
...
- 지원 API를 `srchBooks`, `srchDtlList`, `loanItemSrch`, `recommandList`, `libSrchByBook`로 제한
- `recommandList` ISBN 1~5개 검증과 결과 cache
- `loanItemSrch` 전국·지역 범위만 허용하고 특정 도서관 범위를 노출하지 않음
- 정상·빈 응답
- XML/JSON 오류
- timeout·429·5xx
- 필드 누락
...
- 필수 데이터셋별 schema contract와 매핑 필드 전수 확인
- 이미지·시설 부분 커버리지 행 부재를 오류로 처리하지 않음
- 이미지·시설·프로그램 보조 데이터의 미매칭 행만으로 새 Library를 생성하지 않음
- 이미지 데이터에 없는 출처명·출처 페이지·이용허락 세부값을 추정하지 않음
- 부산 필터
- 좌표 범위
- 평일 시간을 월~금으로만 확장하고 토요일 시간을 일요일로 복제하지 않음
- 일요일 직접 운영시간 부재 시 별도 근거 없으면 unknown 처리
- `00:00` 특수값
- 제공기관코드를 도서관 코드로 오사용하지 않음
- 시설 `False`/`None`/profile 부재 구분
- `has_cafe` 원천 설명 불일치가 검수 항목으로 남는지
- 프로그램 외부 key 안정성
...
- 표준데이터 import
- 운영시간·휴관·통계·선택적 시설 프로필
- 대상 연도 1~12월 공휴일 완전 수집
- 운영표 생성
- LibraryTag 생성
- 검색·상세·이미지 fallback
...
- 연도 전체 공휴일 수집, 운영표·태그·오늘의 추천 재생성 task
- 공식 이미지 license 정보
...
1. 시설 JSON의 `true/false`는 그대로 저장하고, 행 부재·필드 누락을 별도의 미확인 상태로 유지하는지 확인한다.
