# 도서관 나들이 Frontend 개발 명세서 v4

- 문서 버전: 4.0
- 작성 기준일: 2026-07-06
- 대상 애플리케이션: Vue 3 SPA
- 연동 백엔드: Django REST API `/api/v1`
- 최상위 API 기준: `library_outing_frontend_api_contract_v4.0.md`
- 실행·시연 기준: `README.md`
- 페이지 구조 기준: `도서관 나들이 주요 페이지 구조.txt`
- 디자인 기준: `DESIGN.md`
- 도메인 참고: `library_outing_Django_spec_v4.0.md`, `library_outing_ERD_v4.0.md`

## 0. 문서 사용 규칙

### 0.1 목적

이 문서는 API 계약 v4.0 기준으로 도서관 나들이 프론트엔드의 최종 구현 범위, Route, Service, Store, DTO, UI 상태, 사용자 행동, 검증 기준을 정의한다.

과거 문서는 보존하되, v4 구현 판단은 이 문서를 우선한다.
...
2. `library_outing_frontend_api_contract_v4.0.md`
3. `README.md`의 실행·fixture·smoke test 순서
4. `library_outing_Django_spec_v4.0.md`, `library_outing_ERD_v4.0.md`
5. 본 Frontend 명세 v4
6. `DESIGN.md`
7. 과거 명세와 목업

API 계약에 없는 Endpoint, Request, Response field는 프론트에서 만들지 않는다. 단, API 계약 v4.0에 포함된 기능은 현재 구현 대상으로 취급한다.
...
- AI/GMS는 프론트에서 직접 호출하지 않는다.
- 추천 흐름은 `Vue → Django API → RecommendationProvider → Django validation → Vue`를 따른다.
- 프론트는 백엔드가 검증해 반환한 AI 추천 결과만 표시한다.
- AI/GMS가 도서관 존재 여부, 시설 여부, 운영 여부, 책·프로그램·후기 존재 여부, 이미지·출처·라이선스 같은 원천 사실을 생성하거나 수정한다고 가정하지 않는다.
...
기존 `/reviews`는 v4.0에서 커뮤니티 목록 Route가 아니다. 필요하면 `/reviews`를 `/community`로 redirect한다.
...
- API v4.0이 comma string을 명시한 필터는 배열 UI를 comma string으로 직렬화한다.
...
- `ai_confidence`는 사용자에게 점수처럼 과하게 노출하지 않는다. 필요하면 개발용 디버그 또는 보조 정보로만 취급한다.
- 프론트는 `ai_rank`를 표시용 순서로 사용할 수 있지만 순위 근거를 자체 계산하지 않는다.
...
API v4.0 주의사항에 따라 `is_saved`, `is_liked` 필드에 의존하지 않는다.
...
`도서관 나들이 주요 페이지 구조.txt`의 모든 상위 페이지는 v4.0 최종 구현 대상이다.
...
다만 다음 항목은 API v4.0 제약에 맞게 조정한다.

- 늦게까지 운영 `18:00 이후` 필터는 `late_open_after`가 unsupported이므로 직접 query로 구현하지 않는다.
- 후기 태그 옵션 공급은 API v4.0에 전용 options endpoint가 없으므로 seed/tag 계약 확인 후 구현한다.
...
각 service는 API v4.0 query allowlist를 갖고, unsupported query를 보내지 않는다.
...
- AI 추천 provider 호출, prompt, schema validation, fallback 판단은 모두 Django가 담당한다.
...
- 프로그램 DTO의 display field와 comma filter를 v4.0 기준으로 조정했다.
- 선호 설정과 행동 기반 성향의 역할을 분리했다.

## 26. 구현 순서 권장

1. API v4.0 service allowlist 정리
2. 공통 media URL, image fallback, attribution overlay 정리
3. 커뮤니티 목록/상세
4. 나의 나들이 dashboard
5. 도서관 고급 필터와 상세 related/similar
6. 인기 도서와 책 v4.0 보강
7. 프로그램 v4.0 필터/display 보강
8. 선호 설정
