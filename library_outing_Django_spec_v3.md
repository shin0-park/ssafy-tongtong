...
서비스 내부 프로그램 신청·예약·결제·참여 이력, 실시간 열람실 잔여좌석, 후기 자동 텍스트 태깅은 범위에서 제외한다. v4에서는 AI가 사용자 성향 분석, 우선 태그 산출, 후보 재랭킹, 추천 이유 생성에 직접 참여할 수 있다.
...
- GMS가 없어도 mock/fallback provider로 핵심 추천 흐름이 모두 동작해야 한다.
...
AI_RECOMMENDATION_MIN_CANDIDATE_LIMIT=10
AI_RECOMMENDATION_CACHE_SECONDS=900
...
후기 선택지는 ERD v4.0의 확정 목록을 seed한다. 모든 항목은 `semantic_kind=experience`, `is_review_selectable=True`다.
...
### 9.2.2 AI Recommendation Layer v4

AI Recommendation Layer는 Django 내부 service 경계에서만 호출한다. Vue는 AI/GMS key를 직접 사용하지 않는다.
...
- 전체 도서관 raw data 전달
- 원천에 없는 도서관, 시설, 운영, 책, 프로그램, 후기 사실 생성 요청

출력 JSON:

```json
{
  "priority_purposes": [{"code": "study", "weight": 0.5}],
  "priority_tags": [{"code": "facility_lounge", "weight": 0.7}],
  "preferred_regions": [{"sigungu": "사상구", "weight": 0.4}],
  "weights": {
    "purpose": 0.3,
    "tag": 0.4,
    "region": 0.2,
...
Django가 planner의 우선 태그와 가중치를 이용해 후보 도서관을 10~20개로 줄인다. 시설, 운영 여부, 장서 수, 좌석 수, 지역, 거리 계산은 DB와 기존 service가 담당한다.

후보 feature bundle에는 다음 요약만 포함한다.
...
입력은 DB Retrieval 후보 10~20개의 요약 feature로 제한한다.
  ...
  "items": [
    {
      ...
      "rank": 1,
      "confidence": 0.82,
      "matched_priority_tags": ["facility_lounge", "review_quiet"],
      "recommendation_reason": "조용한 학습공간과 휴게공간 선호가 함께 맞아요."
    }
  ]
}
```
...
AI 출력은 응답에 사용하기 전에 반드시 검증한다.

- `library_id` 존재 확인
- `tag_code` 존재 확인
- 비활성 도서관 제거
- 운영 필수 조건 위반 후보 제거
- JSON schema 위반 시 fallback 처리
- `recommendation_reason`은 입력 feature에 있는 근거만 사용하도록 검증한다.

AI는 도서관 존재 여부, 시설 여부, 운영 여부, 책·프로그램·후기 존재 여부, 이미지·출처·라이선스 같은 원천 사실을 생성하거나 DB에 반영하지 않는다.

#### RecommendationProvider 구조

```python
class RecommendationProvider(Protocol):
    def plan_preferences(self, context: PreferencePlanningContext) -> PreferencePlan:
        ...

    def rerank_libraries(self, bundle: LibraryCandidateFeatureBundle) -> RerankResult:
        ...
```

Provider:

- `GMSChatRecommendationProvider`: `GMS_OPENAI_BASE_URL/chat/completions`를 Django에서 호출한다.
- `MockRecommendationProvider`: key 없이 개발·시연 가능한 결정론적 JSON을 반환한다.
- `RuleBasedFallbackRecommendationProvider`: 기존 rule-based 점수를 fallback/baseline으로 사용한다.
- `FineTunedRecommendationProvider`: 추후 fine-tuned model로 교체 가능한 확장 지점이다.

Fallback:

- `GMS_API_KEY` 없음, 만료, timeout, quota 초과, schema 위반, validation 실패 시 fallback provider로 즉시 전환한다.
...
```json
{
  ...
  "personalized": {
    ...
    "is_available": true,
    "priority_tags": [
      {"code": "facility_lounge", "label": "휴게공간", "weight": 0.7}
    ],
    "fallback_used": false,
    "items": []
  },
...
- `personalized.priority_tags`: AI Preference Planner 또는 fallback provider가 산출한 우선 태그를 검증 후 반환한다.
- `personalized.fallback_used`: AI provider 실패 또는 검증 실패로 mock/rule-based fallback을 사용한 경우 `true`다.
- personalized item은 `ai_rank`, `ai_confidence`, `matched_priority_tags`, `recommendation_reason`을 포함할 수 있다.
...
- 대표 문장은 상위 축과 태그를 이용한 결정론적 template이 기본이다. v4 추천 판단은 AI Recommendation Layer가 별도 provider 경계에서 담당하며, 대표 문장 보조가 필요하면 원천 사실을 생성하지 않는 별도 표현 보조로만 사용한다.
...
```text
AI Preference Planner
  입력: 사용자 행동 요약, 수동 선호, 사용 가능한 Tag.code 목록
  출력: priority_purposes, priority_tags, preferred_regions, weights

DB Retrieval
  입력: planner 출력 + DB fact/service
  출력: 후보 도서관 10~20개와 요약 feature

AI Reranker
  입력: 후보 요약 feature
  출력: rank, confidence, matched_priority_tags, recommendation_reason

Django Validation
  library_id/tag_code/활성 도서관/운영 필수 조건/schema 검증
```

기존 공개 목적·직접 태그 기반 후보 점수, 수동 선호 보너스, 행동 기반 `UserPreferenceItem` 점수는 DB Retrieval의 baseline score와 fallback provider에서 사용한다.
...
- AI 출력은 영속 저장하지 않고 요청 단위 계산 또는 단기 캐시만 사용한다.
...
### 12.8 GMS / AI provider 경계

GMS와 다른 AI provider는 Django를 경유한다.

```text
Vue → Django → RecommendationProvider → Django validation → Vue
```
...
- 사용자 성향 분석 보조
- 우선 태그·목적·지역·가중치 산출
- Django가 줄인 후보 10~20개의 재랭킹
- 입력 feature에 근거한 추천 이유 생성
- 이미 계산된 나의 나들이 상위 성향의 자연스러운 문장 표현

금지:

- 도서관 운영·시설·소장 사실 생성
- 전체 도서관 raw data를 AI 입력으로 직접 전달
- 후기 태그 자동 저장
- 원천에 없는 근거 생성
- 이미지·출처·라이선스 생성 또는 수정

timeout·오류·quota 초과·key 없음·schema 위반·validation 실패 시 mock 또는 rule-based fallback provider로 즉시 전환한다.
...
- AI Preference Planner 입력에 전체 도서관 raw data가 없음
- AI Reranker 입력이 후보 10~20개 요약 feature로 제한됨
- AI 출력의 library_id/tag_code/schema 검증
- GMS key 없음·만료·timeout에서 mock/rule-based fallback 응답 유지
