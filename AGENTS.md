...
3. **모델, FK, 제약조건, import, 태그·추천 도메인 정책**
   ...
   - `library_outing_Django_spec_v3.md`
...
| 관심사 | 최상위 기준 |
|---|---|
| 모델, FK, `on_delete`, `related_name`, 제약조건, migration | ERD v3, Django 명세 v3, 실제 migration |
| import, 정규화, 외부 식별자, 태그 의미, 추천 도메인 규칙 | Django 명세 v3, ERD v3 |
| 현재 공개 Endpoint·Method·Payload·응답·Status | 프론트 전달용 API 계약 + 실제 URL/serializer 테스트 |
| Frontend Route·Store·Service·Component·UI/UX | Frontend 명세 v2.0 |
| 패키지·런타임·기술 선택 | `SKILLS.md`, 실제 설정 파일 |

Frontend 작업 중 API 계약과 Django v3의 예정 API가 다르면 **현재 공개 API 계약**을 따른다. 이 규칙은 백엔드 모델, migration, serializer 또는 view를 임의로 수정할 권한을 뜻하지 않는다.
...
Backend 장기 설계에는 운영표, 고급 추천, 행동 성향 분석 등이 포함될 수 있다. 그러나 Frontend에는 현재 공개 API에서 실제 제공하는 기능만 노출한다.
...
GMS가 없어도 핵심 기능은 모두 동작해야 한다. GMS는 사실 필드, 시설·운영 여부, 추천 점수 또는 추천 순위를 결정하지 않는다.
...
- Frontend 디렉터리, Route, Store, Service 구조는 Frontend 명세 v2.0을 따른다.
