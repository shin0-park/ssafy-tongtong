...
- 문서 버전: 1.1
- 작성 기준일: 2026-06-21
- 기준 ERD: `library_outing_ERD_revised.md`
...
4. 도서 검색과 소장 도서관·전일 기준 대출 가능 여부 조회
5. 전국·부산·특정 도서관 인기 대출 도서 목록
6. 도서관 프로그램과 개최 도서관 통합 조회
7. 도서관·책·프로그램 저장, 후기, 행동 기반 취향 집계
8. 공휴일·도서관·프로그램·인기 도서 데이터의 예약 수집과 파생 데이터 생성

서비스 목업은 화면 의도를 파악하는 참고자료로 사용한다. 내부 프로그램 신청·예약·결제·알림, 신청 상태 추적, 성향 백분율, AI 요약은 MVP 범위에 포함하지 않는다.

### 고정 정책

- 홈 추천은 실시간 열람실 API를 사용하지 않는다.
- 열람실 API는 도서관 상세 화면에서만 호출한다.
- 열람실 최신 성공 스냅샷의 `fetched_at`이 1시간 이내이면 외부 호출 없이 재사용한다.
- 브라우저 현재 좌표는 해당 HTTP 요청에서만 사용하고 DB에 저장하지 않는다.
- 정보나루 대출 가능 여부는 전일 기준이라고 응답에 명시한다.
- 책 둘러보기의 목록형 콘텐츠는 인기 대출 도서만 제공한다.
- 프로그램은 내부 DB에서 조회하며 신청 상태와 내부 신청 이력을 관리하지 않는다.
- 시설 정보는 운영자가 공식 근거를 확인해 수동 입력한다.
- 공휴일은 공식 특일 API에서 수집해 DB에 저장한다.
- 도서관 운영 여부는 공휴일·정기 휴관·특정일 예외를 결합한 `LibraryDailySchedule`을 우선 사용한다.
- 홈 상단 기본 추천은 매일 Celery로 생성하며 같은 날짜에는 같은 테마와 기본 결과를 사용한다.
- 행동 기반 개인화는 도서관·책·프로그램 현재 저장 수 합계가 20건 이상일 때만 활성화한다.
- 저장 합계는 활성화 기준으로만 사용하고 취향은 신호 종류별로 따로 계산한다.
...
| Celery | `>=5.6,<5.7` | 예약 수집·운영표·추천·선호 계산 |
...
| `books` | 도서·소장·복본·대출 가능·인기 도서 스냅샷 |
| `programs` | 프로그램·회차·검색 |
| `recommendations` | 방문 목적, 일일 추천 테마, 기본 추천 생성, 개인화 재정렬 |
| `interactions` | 저장, 후기, 개인화 준비 상태와 사용자 취향 |
...
PERSONALIZATION_MAX_BONUS_RATIO=0.25
...
- URL 전체를 로그로 남길 때 `authKey`, `serviceKey`, `ServiceKey`를 마스킹한다.
...
#### LibraryOpeningHour, LibraryClosureRule, PublicHoliday, LibraryDailySchedule

- `LibraryOpeningHour.day_type`, `day_of_week`, `specific_date` 조합에 `CheckConstraint` 적용
- `schedule_status="open"`이면 open/close time 필수
- 같은 날짜에 여러 구간이 필요할 수 있으므로 `sequence` 사용
- 자정 이후 종료는 `closes_next_day=True`
- `LibraryClosureRule.rule_type`: `weekly`, `nth_weekday`, `public_holiday`, `named_holiday`, `temporary`, `full_closure`, `unknown`
- `normalized_rule`은 요일·주차·공휴일 코드·예외요일을 JSON으로 보존
- `PublicHoliday`: `(date, holiday_code)` unique
- `LibraryDailySchedule`: `(library, date)` unique, 상태 `open|closed|unknown`
- `LibraryDailySchedule`은 파생 데이터이므로 운영 규칙이나 공휴일이 변경되면 해당 범위를 다시 생성
- 운영 판정 우선순위:
  1. `specific_date` 명시 일정
  2. 임시·장기 휴관
  3. 특정 명절 또는 전체 공휴일 휴관
  4. 주차별·요일별 정기 휴관
  5. 일반 요일 운영시간
  6. 판정 불가능 시 `unknown`
...
- 시설은 운영자 수동 입력을 기본으로 하고 검수 필드·공식 근거 URL·확인 시각 보존
- 시설 행이 없으면 `미확인`으로 취급하고 검색에는 검증된 긍정 행만 사용
...
- `source_status_raw`: 원천 상태 원문
- `is_canceled`: 취소가 명시된 경우 true
- `source_url`: 공식 원문 페이지
- `content_hash`: `CharField(max_length=64)`
- unique: `(source, external_program_id)`
- 인덱스:
  - `(library, start_at)`
  - `(is_visible, start_at)`
  - `(is_canceled, start_at)`
  - `target_codes` JSONB GIN 선택

신청 시작·종료, 신청 URL, 신청 가능 여부, 내부 신청 이력 필드는 만들지 않는다.

응답용 `schedule_status`는 다음 순서로 계산한다.

1. `is_canceled=True`이면 `canceled`
2. `end_at < now`이면 `ended`
3. `start_at <= now <= end_at`이면 `ongoing`
4. `start_at > now`이면 `scheduled`
5. 날짜가 부족하면 `unknown`

#### ProgramSession

반복 프로그램에서만 생성한다. 단일 회차는 Program의 start/end만 사용한다.
...
- 인기 목록 범위는 `national`, `region`, `library`만 허용
- `query_hash`는 auth key·pageNo를 제외한 정규화 쿼리의 SHA-256
- `query_params`의 key와 다중값을 정렬한 뒤 해시
...
- 활성 테마는 `display_order` 순으로 날짜에 따라 순환
- 같은 날짜의 테마는 모든 요청에서 동일해야 하며 요청마다 random 선택하지 않음
- 테마 규칙은 목적 규칙과 독립적으로 관리

#### DailyLibraryRecommendationSet, DailyLibraryRecommendationItem

- 기본 후보 상위 `DAILY_RECOMMENDATION_CANDIDATE_LIMIT`개를 매일 저장
- `region_key`를 비어 있지 않은 문자열로 정규화하고 set unique는 추천 날짜·테마·`region_key`·알고리즘 버전
- item unique: `(set, rank)`, `(set, library)`
- 사용자별 결과는 저장하지 않고 요청 시 기본 후보를 재정렬
- 응답에는 `recommendation_date`, `theme`, `algorithm_version` 포함

### 6.8 interactions

- 저장 모델 unique `(user, target)`
- `UserProgramSave`는 `user`, `program`, `memo`만 보유하며 북마크 의미로 사용
- `UserReview.rating`: 1~5 check constraint
- `UserReviewImage`: 최대 5장, 파일당 크기 제한
- `UserPreference`: `OneToOneField(User)`
- `UserPreference.status`: `collecting`, `pending`, `ready`, `failed`
- `signal_count`: 활성 도서관·책·프로그램 저장 수 합계
- `library_signal_count`, `book_signal_count`, `program_signal_count` 별도 저장
- `UserPreferenceItem`: unique `(user_preference, item_type, item_code)`
- 최소 신호 수는 `PERSONALIZATION_MIN_SIGNALS` 설정을 사용
- 성향 백분율·AI 요약용 필드는 MVP에서 만들지 않음
...
부산 프로그램은 하나의 통합 API가 아니라 구·도서관별 제공처가 섞여 있으므로 provider pattern을 사용한다.
...
normalizer는 프로그램명, 개최 도서관, 분류, 대상, 일정, 장소, 비용, 공식 원문 URL만 공통 필드로 만든다. 신청기간과 신청 상태는 수집하지 않는다.

동일 프로그램 병합은 `source + external_program_id`를 1차 키로 사용한다. 서로 다른 원천의 중복 프로그램은 운영자 검수 없이 강제 병합하지 않는다.

### 7.5 PublicHolidayClient

한국천문연구원 특일 정보의 공휴일 조회 기능을 연도별로 수집한다.

```python
class PublicHolidayClient:
    def fetch_public_holidays(self, year: int, month: int | None = None) -> list[NormalizedHoliday]: ...
```
...
신규 임시공휴일 반영을 위해 이미 적재한 연도도 주기적으로 다시 확인한다.
...
| GET | `/me/dashboard/` | 필요 | 저장 수·후기·개인화 준비 상태 요약 |
...
`/me/dashboard/`는 MVP에서 저장한 도서관·책·프로그램 수, 후기 수, 개인화 상태만 보장한다. 성향 백분율과 AI 요약은 반환하지 않는다.
...
홈 상단의 오늘 추천 TOP 1~3을 반환한다.

Query:

| 이름 | 필수 | 설명 |
|---|---|---|
| sido / sigungu | 아니오 | 추천 지역 |
| lat / lng | 아니오 | 거리 계산용 현재 좌표, 저장하지 않음 |
...
1. 오늘 날짜·지역의 `DailyLibraryRecommendationSet` 조회
2. 없으면 비동기 생성 상태를 확인하고 마지막 성공 세트를 fallback으로 사용하거나 동기 최소 계산
3. 비로그인 또는 개인화 미충족 사용자는 기본 순위 TOP 3 반환
4. 인증 사용자의 저장 합계가 20건 이상이고 `UserPreference.status=ready`이면 기본 후보에 개인화 보너스를 더해 TOP 3 재정렬
5. 개인화 계산 대기·실패 시 기본 순위 반환

응답 예시:

```json
{
  "recommendation_date": "2026-06-21",
  "theme": {
    "code": "late_open",
    "label": "오늘은 늦게까지 이용하기 좋은 도서관"
  },
  "region": {"sido": "부산광역시", "sigungu": null},
  "algorithm_version": "daily-v1",
  "personalization": {
    "enabled": true,
    "status": "ready",
    "signal_count": 23,
  ...
  "results": [
    {
      "rank": 1,
      "library": {"id": 101, "name": "연제도서관", "main_image": null},
      "score": 88.4,
      "distance_m": 2310,
      "reasons": ["오늘 늦게까지 운영해요", "선호 지역과 가까워요"]
    }
  ]
}
```
...
6개 방문 목적 중 하나를 선택한 추천 목록을 반환한다.
...
| lat / lng | 아니오 | 현재 좌표, 저장하지 않음 |
| limit | 아니오 | 기본 6, 최대 20 |

목적별 기본 점수를 계산하고 개인화 준비 상태가 `ready`인 사용자에게만 제한된 취향 보너스를 적용한다. 이 endpoint에서도 열람실 실시간 API를 호출하지 않는다.

응답에는 `purpose`, `region`, `algorithm_version`, `personalization`, `results`를 포함한다.
...
`open_now`, `weekend_open`, `holiday_open`은 우선 `LibraryDailySchedule`에서 조회한다. 대상 날짜의 파생 행이 없으면 운영시간·휴관 규칙으로 계산하되 외부 API는 호출하지 않는다.
...
- 일반 검색 결과에는 소장 도서관 요약을 강제로 붙이지 않음
- 소장 도서관은 사용자가 `도서관 보기`를 선택했을 때 별도 endpoint에서 조회
...
- `availability_limit`: 기본 10, 최대 20
...
5. `include_availability=true`일 때 상위 `availability_limit`개 도서관에만 `bookExist` 호출
6. 동시 호출은 semaphore와 provider rate limit 적용
7. 각 availability는 24시간 캐시

응답의 `loan_available`에는 `availability_basis="previous_day"`, `source_effective_date`, `checked_at`을 포함한다. 조회하지 않은 도서관은 `availability_status="not_requested"`로 구분한다.
...
| region / detail_region | 지역 범위 조건 |
| library_id | 도서관 범위 조건 |
| start_date / end_date | 집계 기간 |
| age / gender / kdc / detail_kdc | 선택 조건 |
| limit | 기본 20, 최대 50 |

검증:

- `scope=region`이면 `region` 필수
- `scope=library`이면 `library_id` 필수
- `scope=national`이면 지역·도서관 범위 조건 금지

최신 24시간 이내 `PopularBookSnapshot`을 반환한다. 메인 노출용 전국·부산 인기 목록은 Celery가 매일 미리 생성한다.
...
- `target`
- `start_from`, `start_to`
- `ongoing_on`: 특정 날짜에 진행되는 프로그램
- `ordering=start_at|-start_at|title`

외부 API를 호출하지 않는다. 기본 목록은 종료되지 않은 프로그램을 우선 표시하며 신청 가능 여부로 필터링하지 않는다.
...
프로그램 상세, 회차, 개최 도서관, 공식 원문 URL, 원천 표시, 마지막 동기화 시각을 반환한다. 신청 상태와 신청 이력은 반환하지 않는다.
...
| GET | `/me/saved-programs/` | 관심 프로그램 북마크 |
| PUT/DELETE | `/me/saved-programs/{program_id}/` | 저장·메모/해제 |
| GET | `/me/reviews/` | 작성 후기 |
| PATCH/DELETE | `/me/reviews/{review_id}/` | 본인 후기 수정/삭제 |
| GET | `/me/preference/` | 개인화 준비 상태와 계산된 상위 취향 항목 |

저장 변경 후 다음 규칙을 적용한다.

1. transaction commit 이후 세 저장 수의 합계를 계산
2. 20건 미만이면 `collecting`으로 갱신하고 기존 취향 항목을 제거한 뒤 계산 task를 실행하지 않음
3. 20건 이상이면 `pending`으로 갱신하고 30~60초 debounce 후 Celery task 예약
4. 계산 성공 시 `ready`, 실패 시 `failed`
5. 계산 중이거나 실패해도 추천 API는 기본 추천을 정상 반환
...
```json
{
  ...
  "signal_count": 13,
  "required_count": 20,
  "breakdown": {"libraries": 5, "books": 6, "programs": 2},
  "calculated_at": null,
  "items": []
}
```
...
- 활성·검증된 `LibraryFacility`, `LibraryTag`
- 향후 30일 프로그램 수
- 후기 평균·개수
- 공식 이미지 유무
- 요청 좌표와 계산한 거리
- 준비 완료 사용자의 `UserPreferenceItem`
...
활성 `DailyRecommendationTheme`을 `display_order`로 정렬하고 다음과 같이 날짜별 테마를 결정한다.

```python
theme = active_themes[target_date.toordinal() % len(active_themes)]
```

페이지 새로고침마다 random 테마를 고르지 않는다.

Celery는 지역별 후보를 기본 점수 순으로 최대 20개 저장한다. 공개 응답은 기본 상위 3개이며, 준비 완료 사용자는 같은 후보 집합 안에서만 개인화 재정렬한다.
...
| 목적 | 핵심 feature |
|---|---|
| study | 정적 열람좌석 수, 스터디룸, 조용한 공간, 늦은 폐관, Wi-Fi |
| book | 도서 자료 수, 자료실·디지털실, 정보나루 연계 여부, 사용자 KDC 선호 |
| kids | 어린이자료실, 수유실, 어린이 대상 프로그램, 가족 편의시설 |
| program | 향후 프로그램 수, 선호 프로그램 분류 일치 |
| mood | 공식 내부 이미지, 자연채광·카페·라운지·전시공간, 후기 평점 |
| nearby | 거리, 해당 날짜 운영 여부, 주차·접근성 태그 |
...
+ UserBookSave 수
+ UserProgramSave 수
>= PERSONALIZATION_MIN_SIGNALS  # 기본 20
```

- 합계는 활성화 여부 판단에만 사용
- 도서관 저장은 지역·시설·태그 선호 계산
- 책 저장은 KDC·도서 주제 선호 계산
- 프로그램 저장은 프로그램 분류·대상·개최 도서관 선호 계산
- 한 종류의 저장만 20건인 경우 그 종류에서 추론할 수 있는 항목만 사용

점수:

```text
personalization_ratio = min(PERSONALIZATION_MAX_BONUS_RATIO, 0.25)
final_score = base_score × (1 - personalization_ratio)
            + preference_score × personalization_ratio
```

개인화 결과가 없거나 `status != ready`이면 `final_score = base_score`로 처리한다.

### 9.5 추천 사유와 동점 처리

- 추천 사유는 점수 기여도가 큰 상위 2~3개 규칙에서 생성
- 개인화 사유는 실제로 일치한 취향 항목이 있을 때만 추가
- 결측값은 규칙의 `missing` 정책 적용
...
1. scope와 query params 정규화
2. `query_hash` 계산
3. 최신 24시간 이내 snapshot 확인
4. 필요 시 정보나루 인기 API 호출
5. Book bulk upsert
6. snapshot + items transaction 저장
7. 이전 snapshot은 보존정책에 따라 정리

동일 snapshot 생성 race는 Redis lock `popular-books:{scope}:{query_hash}`로 제어한다.

### 10.5 일자별 운영표

- API 요청 시 매번 복잡한 휴관 규칙을 파싱하지 않는다.
- Celery가 기본적으로 오늘부터 향후 180일을 생성한다.
- `LibraryOpeningHour`, `LibraryClosureRule`, `PublicHoliday`가 변경되면 영향 날짜 범위를 다시 생성한다.
- `open_now`는 오늘의 `LibraryDailySchedule`과 현재 시각을 조합한다.
- 파생 행이 없거나 규칙이 충돌하면 `unknown`을 반환하고 임의로 open 처리하지 않는다.

### 10.6 일일 추천

- `DailyLibraryRecommendationSet`은 지역·날짜·테마별로 1개만 생성
- Celery lock: `daily-recommendation:{date}:{region}`
- 생성 실패 시 최근 성공 세트를 fallback으로 사용할 수 있으나 응답에 추천 날짜를 그대로 표시
- 사용자 개인화 결과는 별도 장기 캐시하지 않으며 필요하면 사용자 ID와 추천 날짜 기준의 짧은 Redis TTL만 사용
...
| `sources.sync_public_holidays` | 주 1회 | 현재·다음 연도 공휴일 갱신 |
| `libraries.generate_daily_schedules` | 매일 00:30 + 원천 변경 후 | 향후 180일 운영표 생성 |
| `recommendations.build_daily_library_recommendations` | 매일 00:10 | 날짜별 테마와 지역별 기본 후보 생성 |
| `programs.sync_all_providers` | 6시간마다 | 모든 프로그램 provider 동기화 |
| `books.refresh_popular_book_lists` | 매일 02:00 | 전국·부산·주요 도서관 인기 목록 |
| `books.cleanup_old_snapshots` | 매일 03:00 | 정책 초과 인기·대출가능 snapshot 정리 |
| `libraries.cleanup_room_snapshots` | 매일 03:20 | 1~7일 초과 열람실 원본 정리 |
| `interactions.rebuild_user_preference` | 저장 변경 후 30~60초 debounce | 20건 이상 사용자의 취향 재계산 |
| `interactions.reconcile_personalization_state` | 매일 01:00 | 누락·실패·저장 해제 상태 보정 |
| `libraries.verify_media_assets` | 월 1회 | 링크·라이선스 점검 |
| `sources.audit_unmatched_records` | 매일/수동 | 미매칭 원천 행 보고서 |

작업은 같은 입력으로 여러 번 실행해도 결과가 깨지지 않는 upsert 방식으로 구현한다. 외부 API task에는 timeout, 지수 backoff, 제공처별 rate limit, 분산 lock, `SourceSyncRun` 기록을 적용한다.
...
외부 제공처별 rate limit을 큐 또는 task annotation으로 분리한다. 모든 스케줄은 `Asia/Seoul` 기준으로 운영하고 자정에 작업이 몰리지 않도록 시작 시각을 분산한다.
...
→ 영향 도서관의 DailySchedule 재생성 task 예약
→ current 행 전환
→ sync 결과 집계
```
...
- 평일 시간은 월~금 행으로 확장
- 토요일은 토요일 행
- 일요일은 원천에 명시가 없으므로 휴관 원문과 공식 홈페이지 보강 없이 임의 open 처리하지 않음
- 공휴일 `00:00~00:00`은 `closed` 후보지만 휴관 원문과 함께 판정
- `월+화+공휴일`, `첫째·셋째 월요일`, `설·추석`, `공휴일(일 제외)` 등을 구조화 규칙으로 파싱
- 복잡한 문구나 `휴관중`, `임시 휴관`은 raw text를 보존하고 파싱 실패 시 `unknown`
- 운영시간·휴관 규칙 반영 후 향후 180일 `LibraryDailySchedule` 생성 task 예약
...
MVP에서는 다음 API를 호출하거나 모델에 저장하지 않는다.

- `recommandList`: 마니아·다독자 추천
- `hotTrend`: 급상승 도서
- `newArrivalBook`: 신착 도서
- `usageAnalysisList`의 함께 대출 목록

향후 책 상세 추천 기능이 별도로 기획될 때 새 요구사항으로 검토한다.
...
- 회원가입·토큰 발급 view에는 `AllowAny`를 명시
- 인증: 저장, 후기 작성, 내 취향
- 본인만: 저장 메모, 후기 수정·삭제
...
- 프로그램 원문·도서관 홈페이지 링크는 scheme allowlist `http/https`
...
- 공휴일 달력과 일자별 운영표 read-only 확인
- 특정 날짜 운영표 재생성 admin action
- 대표 이미지 지정
...
- 원천별·개최 도서관별 필터
- visible, deleted, canceled, 일정 범위 필터
- 공식 원문 URL 확인
- 중복 후보 검색
...
- 목적·태그·metric 가중치 편집
- 일일 추천 테마·테마 metric 편집
- 지정 날짜·지역 추천 생성 및 점수 preview admin action
- 개인화 최소 신호 수는 환경설정 값으로 표시
...
- PublicHoliday date+code unique
- LibraryDailySchedule library+date unique
- 일일 추천 set·item unique
- 대표 이미지 1개 제한
...
- `loanItemSrch`, `loanItemSrchByLib`: 순위·대출건수·빈 결과
- 특일 API: 대체공휴일, 같은 날짜 다중 항목, `isHoliday=N`
- 열람실: null 좌석, 방 ID 중복, source timestamp 누락
- 프로그램: 일정 누락, 반복 일정, 취소 원문
...
- `bookExist` effective date가 전날로 저장
- 검색 결과에서 소장 API를 N권 모두 자동 호출하지 않음
- popularity snapshot과 item transaction rollback
- Celery task 재실행 시 중복 행이 생기지 않음

### 17.5 추천 테스트

- 같은 날짜·지역은 같은 일일 테마와 기본 순위
- 날짜가 바뀌면 정의된 순환 규칙에 따라 테마 변경
- 저장 합계 19건은 개인화 비활성
- 저장 합계 20건은 취향 계산 task 예약
- 책만 20권이면 책/KDC 취향만 생성
- `pending`·`failed` 사용자는 기본 추천 정상 반환
- 개인화 보너스가 설정 상한을 넘지 않음
- 실시간 좌석 변경이 홈 추천 결과에 영향 없음
- 추천 이유가 실제 기여 rule과 일치

### 17.6 API 테스트

- 필터 조합과 pagination 상한
- 인증·소유권
- lat/lng validation
- 프로그램 응답에 신청 상태 필드가 없음
- 인기 목록 scope별 필수 파라미터
- API key 미노출
...
- 열람실 API 오류율 20% 초과
- 정보나루 rate limit 증가
- 최근 24시간 인기 도서 snapshot 없음
- 오늘 일일 추천 세트 없음
- 향후 30일 운영표 누락률 임계치 초과
...
- Django 프로젝트, User 모델, PostgreSQL, Redis, Celery Worker·Beat
- 공통 응답·예외·OpenAPI
- DataSource seed와 큐 분리

### Phase 1: 도서관·운영 달력

- 표준데이터 importer
- Library·운영시간·휴관·통계·수동 시설
- PublicHoliday 수집
- LibraryDailySchedule 생성
- 도서관 목록·상세와 관리자 매칭 화면

### Phase 2: 기본 추천

- Purpose·rule seed
- DailyRecommendationTheme·rule seed
- 날짜별 TOP 후보 Celery 생성
- Haversine과 목적별 추천 사유
- 사용자 기본 지역
...
- 도서관·책·프로그램 저장과 후기·이미지
- 20건 threshold와 preference state
- Celery debounce 취향 계산
- 일일·목적 추천의 제한적 개인화
...
- 날짜별 테마와 기본 추천 TOP 1~3이 매일 생성된다.
- 6개 목적 중 하나를 선택하면 DB 기반 추천을 반환한다.
- 저장 합계 20건 미만 사용자는 기본 추천, 20건 이상 준비 완료 사용자는 개인화 추천을 받는다.
- 열람실 실시간 API가 중단되어도 홈이 정상 동작한다.

### 도서관 찾기

- 부산 지역·유형·운영·시설·통계 필터가 DB에서 동작한다.
- 공휴일과 복잡한 휴관 규칙을 반영한 일자별 운영 여부가 표시된다.
- 거리순은 좌표 보유 행에 대해 계산된다.
- 시설은 수동 검수된 긍정 정보만 검색에 사용된다.
...
- 소장 도서관은 별도 요청에서 조회된다.
- 대출 가능 여부에 전일 기준일이 노출된다.
- 전국·부산·특정 도서관 인기 목록이 snapshot에서 조회된다.

### 프로그램

- 내부 DB에서 지역·도서관·분류·대상·기간 필터가 가능하다.
- 원천에서 사라진 프로그램은 FK를 깨지 않고 soft delete된다.
- 프로그램은 조회와 관심 저장만 제공하며 신청 상태를 관리하지 않는다.

### 나의 나들이

- 도서관·책·프로그램 저장과 해제가 가능하다.
- 후기와 이미지 작성·수정·삭제가 가능하다.
- 개인화 준비 상태와 저장 수 breakdown을 확인할 수 있다.
- 취향 계산 실패 중에도 기본 추천이 정상 제공된다.

### 데이터 운영

- Celery Beat가 공휴일·운영표·일일 추천·프로그램·인기 도서 갱신을 예약 실행한다.
- 모든 외부 코드에 원천이 연결된다.
...
4. 특일 API 응답의 대체·임시공휴일 코드 정규화 규칙
5. 표준데이터의 부산 대상 범위를 공공·작은·어린이 중 어디까지 노출할지
6. 도서관 홈페이지 크롤링 허용 여부와 robots/이용약관
7. 공공누리 이미지의 자체 저장·리사이즈 가능 범위
8. 사용자 후기 1인 1도서관 1건 제한 여부
9. JWT 방식과 same-origin session 방식 중 최종 프론트 구조
