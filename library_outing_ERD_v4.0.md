...
v4는 실제 Django 앱과 `models.py` 생성을 바로 시작할 수 있도록 v3의 필드·관계·삭제 정책과 AI 추천 경계를 다시 검토한 결과다. 기존 데이터셋·공휴일·운영표·프로그램 상태 정책은 유지하고, AI Recommendation Layer가 요청 단위 추천 판단에 참여할 수 있도록 다음을 정정한다.
...
41. v4에서는 AI가 사용자 성향 분석, 우선 태그 산출, 후보 재랭킹, 추천 이유 생성에 사용할 수 있다.
42. AI 추천 흐름은 `AI Preference Planner → DB Retrieval → AI Reranker → Django Validation`이다.
43. 전체 도서관 raw data를 AI에 직접 전달하지 않는다. Planner 입력은 사용자 요약, 수동 선호, 사용 가능한 `Tag.code` 목록으로 제한하고, Reranker 입력은 후보 10~20개의 요약 feature로 제한한다.
44. AI 출력은 요청 단위 추천 `rank`, `confidence`, `priority_tags`, `matched_priority_tags`, `recommendation_reason`으로 사용할 수 있다.
45. AI 출력은 도서관 존재 여부, 시설 여부, 운영 여부, 책·프로그램·후기 존재 여부, 이미지·출처·라이선스 같은 원천 사실 필드에 반영하지 않는다.
...
37. AI/GMS 출력은 사실 필드·시설·운영 여부·이미지·출처·라이선스에 반영하지 않는다. 단 v4 AI Recommendation Layer의 검증된 출력은 요청 단위 추천 순위, 신뢰도, 우선 태그, 추천 이유로 사용할 수 있다.
...
`여기는 어때요?`는 MVP에서 별도 결과 테이블을 만들지 않는다.

1. AI Preference Planner는 사용자 행동 요약, 수동 선호, 사용 가능한 `Tag.code` 목록을 입력받아 우선 목적·태그·지역·가중치를 산출한다.
2. Django DB Retrieval은 planner 출력과 DB fact/service를 이용해 추천일 `open` 후보에서 오늘의 추천을 제외하고 후보 10~20개를 만든다.
3. AI Reranker는 후보 요약 feature만 입력받아 요청 단위 rank, confidence, matched priority tags, recommendation reason을 산출한다.
4. Django Validation은 `library_id`, `tag_code`, 활성 도서관, 운영 필수 조건, JSON schema를 검증한다.
5. 결과가 부족해도 휴관·미확인 후보로 채우지 않는다.
6. 사용자 데이터 변경 시 개인화 캐시만 무효화한다.
7. GMS key 없음·만료·timeout·schema 위반·검증 실패 시 mock 또는 rule-based fallback provider를 사용한다.

AI 추천 결과는 MVP에서 영속 저장하지 않고 요청 단위 계산 또는 user/date/version 기준 단기 캐시로 처리한다.

#### AI 추천 로그 모델 보류

MVP 이후 운영 관측과 품질 평가가 필요하면 다음 모델을 별도 확장으로 둘 수 있다. v4 MVP에는 생성하지 않는다.

- `AIRecommendationRun`: provider, schema version, request context hash, fallback 여부, latency, error code
- `AIRecommendationLog`: run FK, 검증 전후 item 수, 제거 사유 집계, 최종 library id와 rank snapshot

이 모델을 추가하더라도 AI 출력은 원천 사실 필드나 도메인 canonical 데이터에 반영하지 않는다.
...
| 여기는 어때요? | 요청 시 계산 | 사용자별 단기 캐시만 | AI 추천 결과는 영속 저장하지 않음, 오늘의 추천과 중복 없는 최대 3개 |
